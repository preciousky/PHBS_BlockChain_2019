...
public class TxHandler {
    ...
    /**
     * Creates a public ledger whose current main.main.UTXOPool (collection of unspent transaction outputs) is
     * {@code utxoPool}. This should make a copy of utxoPool by using the main.main.UTXOPool(main.main.UTXOPool uPool)
     * constructor.
     */
    ...
    /**
     * @return true if:
     * (1) all outputs claimed by {@code tx} are in the current main.main.UTXO pool,
     * (2) the signatures on each input of {@code tx} are valid,
     * (3) no main.main.UTXO is claimed multiple times by {@code tx},
    ...
    public boolean isValidTx(Transaction tx) {
        ...
        double outputSum = 0;
        double inputSum = 0;
        // A buffer of main.main.UTXO to confirm there is no multiple claims inputs
        UTXOPool bufferPool = new UTXOPool();
        /*
        make sure all inputs are available
        1. included in the main.main.UTXO for (1)
        2. get the sum of them for (5)
        3. no multiple claimed main.main.UTXO for (3)
        4. the sig is valid for (2)
         */
        for (int i = 0; i < tx.numInputs(); i++) {
            Transaction.Input in = tx.getInput(i);
            // package the input into a main.main.UTXO
            UTXO u = new UTXO(in.prevTxHash, in.outputIndex);
            System.out.println("############====----1----====#############");
            if (!pool.contains(u)) {
                System.out.println("The prevHash in input[" + i + "] is not contained in main.UTXOPool, return false!");
                System.out.println("#######################Finish the isValid function.#######################");
                return false;
            }
...
//        System.out.println("Sum of Outputs:" + outputSum);
     ...
     * updating the current main.main.UTXO pool as appropriate.
     */
