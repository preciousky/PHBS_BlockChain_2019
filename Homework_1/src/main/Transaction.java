...
public class Transaction {
    ...
    /**
     * obtain the raw data for signing. i.e., i-th input and all outputs
     * @param index indicate the specified index of inputs
     * @return an byte[] of the raw data for signing.
     */
    public byte[] getRawDataToSign(int index) {
        // if index is greater than the size of the inputs array, something must be wrong, then return.
        if (index > inputs.size())
            return null;
        // Get the buffer ready
        ArrayList<Byte> sigData = new ArrayList<Byte>();

        // add the prevTxHash of i-th input into the buffer
        Input in = inputs.get(index);
        byte[] prevTxHash = in.prevTxHash;
        if (prevTxHash != null)
            for (int i = 0; i < prevTxHash.length; i++)
                sigData.add(prevTxHash[i]);
        // add the index of the output generating the input into the buffer
        ByteBuffer b = ByteBuffer.allocate(Integer.SIZE / 8);
        b.putInt(in.outputIndex);
        byte[] outputIndex = b.array();
        for (int i = 0; i < outputIndex.length; i++)
            sigData.add(outputIndex[i]);
        // add all values and addresses of the outputs into the buffer.
        ...
        // restore the buffer as an array
    ...
    /**
     * add a sig for specified input in the transaction
     * @param signature the sig to be added
     * @param index indicate the specified index of input
     */
    ...
    /**
     * get everything of the inputs and outputs in the transaction into an byte[], used in the finalize function.
     * @return an byte[] of the whole transaction
     */
